                ...
                <div class="dropdown d-flex">
                    <button class="btn-account" type="button" data-toggle="dropdown" aria-haspopup="true"
                        ...
                        <span class="user-avatar user-avatar-md">
                            {!! (Auth::guard('teacher')->user()->avatar) ? '<img src="'.userFile(Auth::guard('teacher')->user()->avatar, '', Auth::guard('teacher')->user()->id).'" alt="">' : '<span class="d-block fa fa-user-circle"></span>' !!}
                        </span>
                        <span class="account-summary pr-lg-4 d-none d-lg-block">
                            <span class="account-name">{{ Auth::guard('teacher')->user()->first_name ?? '' }}
                                {{ Auth::guard('teacher')->user()->last_name ?? '' }}</span>
                        </span>
                    </button>
